...
from app.schemas.users import Token, UserCreate, UserLogin, UserOut
...
async def update_weather(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Errore durante l'aggiornamento della temperatura: {str(e)}",
        )
...
async def update_exchange(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Errore durante l'aggiornamento del cambio: {str(e)}",
        )
...
async def get_dashboard(
    ...
    exchange_query = await db.execute(
        select(ExchangeData)
        .filter_by(target_currency="USD")
        .order_by(ExchangeData.timestamp.desc())
        .limit(1)
    )
    ...
    news_query = await db.execute(
        select(NewsArticle).order_by(NewsArticle.timestamp.desc()).limit(3)
    )
    latest_news = news_query.scalars().all()

    return {"weather": latest_weather, "exchange": latest_exchange, "news": latest_news}


@router.get("/weather/history")
async def last_24_hours_temperatures(
    db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    weather_query = await db.execute(
        select(WeatherData).order_by(WeatherData.timestamp.desc()).limit(24)
    )
    ...
    return [
        {
            "time": h.timestamp.astimezone(ZoneInfo("Europe/Rome")).strftime("%H:%M"),
            "temp": h.temperature,
        }
        for h in history
    ]
...
async def effettua_login(login_data: UserLogin, db: AsyncSession = Depends(get_db)):
    ...
    user = await service_user.authenticate_user(
        email=login_data.email, password=login_data.plain_password
    )
